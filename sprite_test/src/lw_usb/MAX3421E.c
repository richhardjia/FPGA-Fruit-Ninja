...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	if(Status != 0)
	{
		xil_printf("XSpi_Transfer Failed in reg_wr");
		return;
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//write reg via SPI
	//read val via SPI
	u8 ReadBuffer[2] = {reg, 0};
	u8 WriteBuffer[2] = {reg, 0};
	Status = XSpi_Transfer(&SpiInstance, WriteBuffer, ReadBuffer, 2);
	if(Status != 0)
	{
		xil_printf("XSpi_Transfer Failed for Reg + 2");
		return;
	}
	//read return code from SPI peripheral 
	//if return code != 0 print an error
